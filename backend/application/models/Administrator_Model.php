<?php
	class Administrator_Model extends CI_Model
	{
		...
		public function get_users($role = FALSE,$username = FALSE, $limit = FALSE, $offset = FALSE)
		{
			if ($limit) {
		  ...
		  public function create_product($post_image)
		{
			...
			if($result==0)
			{
			$code=$district['code'].++$result."/".date('d-m-Y', strtotime($this->input->post('dat')))."/".$this->session->userdata('username');
			}
			else
			{
			$code=$district['code'].++$result."/".date('d-m-Y', strtotime($this->input->post('dat')))."/".$this->session->userdata('username');
			...
			$data = array('name' => $this->input->post('name'),
							...
							'photo_members' => $this->input->post('photo_members'),
		...
		public function insertproductsmultipleImages($data = array()){
       	 $insert = $this->db->insert_batch('album_images',$data);
       	 return $insert?true:false;
    	}

		// Check Product SKU exists
		public function check_sku_exists($sku){
			$query = $this->db->get_where('albums', array('sku' => $sku));
		...
		public function get_products($id = FALSE)
		{
			...
			$query = $this->db->get_where('albums', array('id' => $id));
			return $query->row_array();
		}
