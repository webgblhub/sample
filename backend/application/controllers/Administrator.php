<?php 
	class Administrator extends CI_Controller{
		...
		public function contractdashboard($page = 'contractdashboard'){
		   if (!file_exists(APPPATH.'views/administrator/'.$page.'.php')) {
		    show_404();
		   }
		   $data['title'] = ucfirst($page);
	  ...
	  // Log in Admin
		public function adminLogin(){
			...
			if($this->form_validation->run() === FALSE){
				...
				$user_id = $this->Administrator_Model->adminLogin($email);
				...
				if ($user_id && $user_id->role_id == 1) {
					...
					//Set Message
					$this->session->set_flashdata('success', 'Welcome to super administrator Dashboard.');
					redirect('administrator/dashboard');
				}else if($user_id && $user_id->role_id == 2){
					$user_data = array(
								'user_id' => $user_id->id,
				 				'username' => $user_id->username,
				 				'email' => $user_id->email,
					...
					redirect('administrator/admindashboard');
					...
					//Set Message
					$this->session->set_flashdata('success', 'Welcome to contract photographers Dashboard.');
					redirect('administrator/contractdashboard');
				}else{
		...
		public function add_user($page = 'add-user')
		{
			...
			$data['roles'] = $this->Role_Model->actlists(FALSE);
			
			$data['districts'] = $this->District_Model->actlists(FALSE);
			...
			if($this->form_validation->run() === FALSE){
				...
				$password = password_hash($this->input->post('contact'),PASSWORD_DEFAULT);
				...
				redirect('administrator/users?role='.$this->input->post('role'));
			}
		...
		public function users($offset = 0)
		{
			...
			//$data['users'] = $this->Administrator_Model->get_users($this->input->get('role'),FALSE, $config['per_page'], $offset);
			$data['users'] = $this->Administrator_Model->get_users($this->input->get('role'),FALSE, FALSE, FALSE);
	...
	public function multipleImageUpload($images,$dataID){
		$album = $this->Administrator_Model->update_products($dataID);
		$images == $_FILES['imgFiles'];
        $data = array();
		
			$district = $this->District_Model->get($this->session->userdata('district'));
			
    		$this->db->where('YEAR(datetime) = YEAR(NOW())');
    		$result = $this->db->get('albums')->num_rows();		
        ...
        if(!empty($_FILES['imgFiles']['name'])){
            ...
            for($i = 0; $i < $filesCount; $i++){
                ...
                if($this->upload->do_upload('userFile')){
					...
					if($result==0)
					{
					$code=$district['code'].$result."-".$j++."/".date('d-m-Y', strtotime($this->input->post('dat')))."/".$this->session->userdata('username');
					}
					else
					{
					$code=$district['code'].$result."-".$j++."/".date('d-m-Y', strtotime($this->input->post('dat')))."/".$this->session->userdata('username');
                    ...
                    $uploadData[$i]['album_id'] = $dataID;
					$uploadData[$i]['img_code'] = $code;
					$uploadData[$i]['title'] = $album['name'];
					$uploadData[$i]['description'] = $album['description'];
					$uploadData[$i]['tag'] = $album['tag'];
					$uploadData[$i]['datetime'] = $album['datetime'];
					$uploadData[$i]['place'] = $album['place'];
					$uploadData[$i]['photo_members'] = $album['photo_members'];
...
public function update_password(){
	...
	{
		$cities=$this->Administrator_Model->getCity();
	...
	{
		$cities=$this->Administrator_Model->getCity2();
