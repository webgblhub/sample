...
class Auth extends CI_Controller {
	...
	{
           		if($this->session->userdata('supplier_id')==true)
		{
 ...
 //    	if ($this->form_validation->run() === FALSE)
	...
	{
		$array_items = array('supplier_id','admin_name','admin_email','admin_logged');
...
$actid=base64_encode($is_reg);
			...
			//$body ='hi'; 
			//print_r($message);die(); 
			$body2 = str_replace(array("<firstname>","<notification title>","<actid>"),array($email,$subject,$actid),$body);
			...
			if (send_email($email, $subject, $body2) == true) {
	        	...
	        	$this->session->set_flashdata('success','Your activation message is send to registered email!');
	        	$this->load->view('admin/login');
			}
			else{
	...
	public function supplier_forgot(){
		...
		if ($verify) {
			...
			if (send_email($email, $subject, $body) == true) {
	        	...
	        	if($update_password)
	        	{
			$this->session->set_flashdata('success','New Password is send to your Email!');
	        	$this->load->view('admin/login');
				}else{
	...
	public function activate()
	{
		...
		$this->session->set_flashdata('success','You have Successfully activated your account. Please sign in to access');
		$this->load->view('admin/login');
	}
