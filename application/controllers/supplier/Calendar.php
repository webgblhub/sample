...
defined('BASEPATH') or exit('No direct script access allowed');
	...
	class Calendar extends CI_Controller
	{
	 	...
	 	public function __construct()
		{
			...
			define('CLIENT_REDIRECT_URL', 'http://localhost/edcpro/supplier/calendar/index/');
		...
		public function index()
		{
			if($this->input->get('code')!="") {
				try {
					...
					$redirect = 'http://localhost/edcpro/supplier/calendar/lists';
					redirect(filter_var($redirect, FILTER_SANITIZE_URL) .'/'.$this->session->userdata('storeid'));
					exit();
				}
				catch(Exception $e) {
					echo $e->getMessage();
		...
		public function addEvent()
		{
			...
			$this->session->set_userdata('storeid', base64_encode($this->input->get('switch')));
